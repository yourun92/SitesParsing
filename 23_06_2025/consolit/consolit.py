...
class Parser:
    def __init__(self):
        self.url = 'https://www.consolit.ru/sitemap-iblock-12.xml'
        self.data = []
        self.ind = 0
        self.ua = UserAgent()
    ...
    def parsing_products(self):
        ...
        for url in product_urls:
            try:
                ...
                try:
                    path = ' > '.join([i.text.strip() for i in soup.select_one('.breadcrumb').select('li')])
                except:
                    path = ''

                try:
                    category = [i.text.strip() for i in soup.select_one('.breadcrumb').select('li')][2]
                except:
                    category = ''

                try:
                    availability = soup.select_one('[itemprop="availability"]').text.strip()
                except:
                    availability = ''

                try:
                    image = 'https://www.consolit.ru' + soup.select_one('a[data-fancybox-group="gallery"]')['href']
                except:
                    image = ''

                try:
                    gallery = ' | '.join(['https://www.consolit.ru' + i['src'] for i in soup.select_one('.slides').select('img')])
                except:
                    gallery = ''

                try:
                    main_description = soup.select_one('[name="keywords"]')['content'].strip()
                ...
                try:
                    ...
                    content_clone = copy.copy(content)
                    for unwanted in content_clone.find_all(['table', lambda tag: 'wraps' in tag.get('class', [])]):
                        unwanted.decompose()  # удаляем только из клона

                    description = content_clone.get_text(separator='\n', strip=True)
                except:
                    description = ''
...
correct_path = os.path.join('23_06_2025', 'consolit', 'consolit.xlsx')
