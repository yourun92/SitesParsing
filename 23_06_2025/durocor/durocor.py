...
class Parser:
    def __init__(self):
        self.url = 'https://durocor.ru/sitemap.xml'
        self.data = []
        self.ind = 0
        self.ua = UserAgent()
    ...
    def parsing_products(self):
        ...
        for url in product_urls:
            try:
                ...
                try:
                    meta_title = soup.select_one('meta[property="og:title"]')['content'].strip()
                except:
                    meta_title = ''

                try:
                    title = soup.select_one('[data-goods-field="title"]').text.strip()
                except:
                    title = ''
                ...
                try:
                    image = soup.select_one('.sb-image-square.sb-skeleton__image.sb-skeleton__image_without-radius').select_one('img')['src']
                except:
                    image = ''
                ...
                try:
                    main_description = soup.select_one('[data-goods-field="description"]').text.strip()
                ...
                try:
                    price = soup.select_one('.s-product-page-type-1__old-price').text
                except:
                    price = ''
                ...
                #     elif soup.select_one('.props_table'):
                #         for i in soup.select_one('.props_table').select('tr'):
                #             key = i.select_one('.char_name').text.strip().replace('\t', '')
...
correct_path = os.path.join('23_06_2025', 'durocor', 'durocor.xlsx')
