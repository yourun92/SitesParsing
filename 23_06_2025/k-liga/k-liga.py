...
class Parser:
    def __init__(self):
        self.url = 'https://k-liga.ru/sitemap.xml'
        self.data = []
        self.ind = 0
        self.ua = UserAgent()
    ...
    def collect_all_pages(self):
        ...
        for u in all_urls:
            if float(u.select_one('priority').text) == 1:
                url = u.select_one('loc').text.split('/')
                url.insert(3, 'catalog')
                product_urls.append('/'.join(url))
    ...
    def parsing_products(self):
        ...
        for url in product_urls:
            try:
                ...
                try:
                    meta_title = soup.select_one('.container').select_one('h1').text
                except:
                    meta_title = ''

                try:
                    title = soup.select_one('.container').select_one('h1').text
                except:
                    title = ''

                try:
                    article = soup.select_one('.ProductContent_product__code__NufqB').text.split(':')[-1].strip()
                except:
                    article = ''

                try:
                    path = ' > '.join([i.text.strip() for i in soup.select_one('.Breadcrumbs_breadcrumb__CUpcJ.d-sm-none').select('li')])
                except:
                    path = ''


                try:
                    category = [i.text.strip() for i in soup.select_one('.Breadcrumbs_breadcrumb__CUpcJ.d-sm-none').select('li')][2]
                except:
                    category = ''
                ...
                try:
                    image = soup.select_one('.swiper-slide').select_one('img')['src']
                except:
                    image = ''
                ...
                try:
                    main_description = soup.select_one('.ProductTabs_description__sD0N5').select_one('p').text.strip()
                except:
                    main_description = ''
                ...
                try:
                    characteristics = {}
                    for i in soup.select('.chakra-tabs__tab-panel.css-a5mhaz')[1].select('div'):
                        keys_values = i.select('span')
                        key = keys_values[0].text.strip()
                        value = keys_values[-1].text.strip()
...
correct_path = os.path.join('23_06_2025', 'k-liga', 'k-liga.xlsx')
